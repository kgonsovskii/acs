...
def tobaseraktopic(ac,n,d,ls):
    ...
    try:
           ...
           # for t in ls:
           #  mp('lime','code='+str(t.code)+',mask='+str(t.mask)+',pers_cat='+str(t.pers_cat))
...
def fd_rak(task,ac):
  try:
      ...
      rmp('lime', 'fd_rak ac=' + str(ac)+zp+'DT='+str(dt), 20)
      ls=[]
      lsx=[]
    #for i in range(1,2,1):
      try:
       i=1
       ls = mbch['chskd'].read_all_keys(ac)
       return i, ls
       #if type(lsx)!=type(ls):
       #rmp('yellow','LS NOT LIST',30)

      except Exception as ee:
       mpr('fd_rak I='+str(i),ee)
       rmp('yellow','sleep 10 & i='+str(i),20)
       gevent.sleep(5)
       xflush()
      # dt = mbch['chskd'].get_dt(ac)
...
def freadallkeys(task,ac):
    ...
    k=mbacl[ac]['kpx']
    mbacl[ac]['kpx']='avt'
    rmp('magenta', 'readallkeys ac=' + str(ac),10)
    try:
        ls=[]
        t1 = stofunc.nowtosec('loc')
        i,ls=fd_rak(task,ac)
        if ls==None:
         rmp('yellow',' NONE '*5,30)
        t2 = stofunc.nowtosec('loc')
        d=t2-t1
        if len(ls)>0 and i != None:
         tobaseraktopic(ac,i,d,ls)
         task.phase = 'end'
         task.secmer = 0
         fprgs(task)
         gevent.sleep(3)
...
def timer1s(interval):
    while True:
     ...
     try:
      lcmd=len(lsgcmdmain)
      if lcmd>0 and mgb['cmdlong']==None:
