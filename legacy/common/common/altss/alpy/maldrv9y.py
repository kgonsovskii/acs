...
def crip(ch):
   try:
    ...
    print('crip','ПОПЫТКА СОЗДАТЬ КАНАЛ='+' /ee='+str(ee))
...
def fbrp(es):
   ...
   try:
           fn= mgb['restartsh']
           if n1>=0 and n2>=0:
            rmp('yellow','BRP brp n1='+str(n1)+zp+'n2='+str(n2)+',fn='+fn,20)
            mp('red','brokenpipe')
            rmp('cyan','fbrp ='+fn,30)
            tobase_acerr(-1, es, -10)
            rmp('magenta','WAIT 30',50)
           # asmydelay(30)
            #asyncio.run(asmydelay(3))
            os.system(fn)
            return
...
def formirstartsh(line):
  try:
      pid=str(os.getpid())
      s='sudo kill '+pid+' ; '+line
      appdir = gfunc.getmydir()
      fn = appdir + 'restart9.sh'
      rmp('lime', 'fn=' + fn, 20)
      outp=open(fn,'w')
      outp.write(s)
      outp.flush()
      ...
      mgb['restartsh']=fn
      rmp('magenta','s='+s,20)
      os.system('chmod 0777 '+fn)

  except Exception as ee:
    mpr('formirstartsh',ee)
...
#mp('red','delay 10 start @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@2')
#asyncio.run(asmydelay(10))
#mp('red','delay 10 STOP  ???????????????????????????????????')
...
if '-psdcpmp' not in gprm.keys():
 gprm['-psdcomp']=mgb['comp']
