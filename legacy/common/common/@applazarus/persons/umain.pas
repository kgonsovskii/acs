...
uses
  ...
  StdCtrls, laz.virtualtrees, laz.VTHeaderPopup,zlib,SQLDB, DB, memds,Types,
...
{$R *.lfm}
uses uallog,ushemanode,ulazfunc,upgs,uperskeys,uglsnames,upersdata,utmzvst,undgrf,UPERS1,ualfunc;
    ...
    //  showmessage('prolog2');
     ...
     // showmessage(sl.text);
      umain.mysysinfo.baseaddr:=sl.Values['lebaseip'];
           umain.mysysinfo.basename:=sl.Values['lebasname'];
           umain.mysysinfo.basepsw :=sl.Values['lebasepsw'];
           umain.mysysinfo.username :=sl.Values['lebaseuser'];
...
TIMERSTART.Enabled:=FALSE;
 // showmessage('prolog1');
  ...
  log('l','END OF PROLOG1 11111111111111111111111111111111111111111111111111111111');
   ...
   //showmessage('prolog1 call prolog2');
  prolog2;
...
begin
    ...
    showmessage('FORM1.readkeys='+line);
...
begin
        form_pers1.Visible:=true;
        form1.Top:=0;
        form_pers1.Top:=form1.Height+10;
        form_pers1.left:=200;
        form_pers1.show;
   {
...
var
 ...
 begin
       ...
       s:='update tss_persinfo set bidtree='+myidsh+' where myid='+myid;
       log('y','linkonperson='+s);
       form1.selfupdc(s);
       s:='update tss_persinfo set actual=false'+' where myid='+myid;
       form1.selfupdc(s);
...
procedure TForm1.otladka;
 var
 FileDateTime,s,pt,ts :string;
 DataInf   : TSearchRec;
 sl:tstringlist;
begin
          ...
          caption:=appdir+appexe+' #VERSION AT '+ts;
          sl.text:=readconfig;
          // showmessage('otladka  sl='+sl.text);
           umain.mysysinfo.baseaddr:=sl.Values['lebaseip'];
           umain.mysysinfo.basename:=sl.Values['lebasname'];
           umain.mysysinfo.basepsw :=sl.Values['lebasepsw'];
           umain.mysysinfo.username :=sl.Values['lebaseuser'];
           //showmessage('prolog2 ip='+sl.Values['lebaseip']+'/ username='+ umain.mysysinfo.username);
           //showmessage('prolog2 psw='+umain.mysysinfo.basepsw);
           formpgs.connect;
