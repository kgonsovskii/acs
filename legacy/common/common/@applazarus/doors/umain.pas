...
uses
  ...
  StdCtrls, laz.virtualtrees,base64,SQLDB,Types,uglink,
  TSSMQTTC,dateutils,
  ImgList,
  fpjson, jsonparser,
  udtrans, UniqueInstance,
  ctypes,StrUtils, LCLType, MaskEdit, Buttons,SQLite3Conn;




type tjscnd=record
      typ    :string; //comp,ch,ac,port,genk
      k1     :string;
      kn     :string;
      cmd    :string;
      cid    :string;
...
type
  ...
  TForm1 = class(TForm)
   ...
   function  readfld(line:ansistring):trcfld;
   ...
   // function  selfupd(s: string): string;
    ...
    function  fnewlinks(ndp,cnd:pvirtualnode):boolean;
    function  updsensbploc(bploc,bidport,code:string):string;
    function  faquass(ssmyid:string):boolean;
    function  gettempera:tstringlist;
    procedure femul;
    procedure formirnewemul(cnd:pvirtualnode);
    procedure m1107add(ttg:string);
    function  fnnaloc(cnd:pvirtualnode;dsn:tnewds):pvirtualnode;
    function  fnniloc(cnd:pvirtualnode;dsn:tnewds):pvirtualnode;
    procedure locread0;
    procedure locread1(ndp:pvirtualnode);
    procedure locread2(ndp:pvirtualnode);
    procedure locread3(ndp:pvirtualnode);
    procedure m711addgrp(cnd:pvirtualnode);
    procedure locread2x(ndp:pvirtualnode);
    procedure locread21(ndp:pvirtualnode);
    function  calcng(dbmyid:string):integer;
    procedure readinsidegrp(cnd:pvirtualnode;bp:string);
    procedure mygrpx(cnd:pvirtualnode);
    function  checklinkls(bidloc:string):integer;
    function  checklinksl(bidsens:string):boolean;
    procedure frmjs_ac(cnd:pvirtualnode;jscmd:tjscnd);
    function  findndofbid(bid: integer): pvirtualnode;
   procedure  pcmdprgs(sl:tstringlist);
   procedure  plinksgrponport(ndport,cndl:pvirtualnode);
   function   findndofbidloc(bid: integer): pvirtualnode;
...
{$R *.lfm}
uses  uallog,ualfunc,uwwcomp,uwwch,uwwac,uformhelp,uformalnewloc,ualnewlocsens,uformimportff,umars1,usyslog,ulocm5,uloc2,
  uformvst2,uemul,
  ualgol,uformeditch,uformeditcomp,uformedittmz,uformlistpers,ualdb3,ushowmes;
...
var
 ...
 q:tsqlquery;
 sl:tstringlist;
 b: TSQLite3Connection;
 t: TSQLTransaction;
 begin
      ptb:=appdir+'config/altssconfig.db3';
      sl:=tstringlist.Create;
      b:= TSQLite3Connection.Create(self);
      b.databasename:=ptb;
      b.hostname:=ptb;
     // showmessage('readconfig 1 ptb='+ptb);
      t:= TSQLTransaction.Create(self);
      t.DataBase:=b;
     q:=tsqlquery.Create(self);
     q.DataBase:=b;
     q.Transaction:=t;
     //showmessage('readconfig 2');
...
function TForm1.readfld(LINE:ansistring):trcfld;
...
var
  Data, dataold: pvstrecord;
  nd, pnd: pvirtualnode;
  nump, s, comp, ch, ac, port, sensor, sti,bidport: string;
  sl: TStringList;
  qrx: TSQLQuery;
  llink:string;
  att:boolean;
...
begin
  ...
  comp := dataold^.comp;
  bidport:=inttostr(dataold^.dbmyid);
  ch := dataold^.ch;
  ac := dataold^.ac;
  port := dataold^.port;
  ch := dataold^.ch;
 ...
 //   if  (strtoint(sl.Values['bploc'])>0) and checklinkbploc(strtoint(sl.Values['bploc'])) then llink:=' связь есть!' ;
    llink:='';
    sl.Values['nm1'] :='id='+ sl.Values['myid']+zp+'bploc='+sl.Values['bploc'];
    s := sl.Values['nm1'] + ',' + sl.Values['actual'];
    sl.Values['sti'] := '-1';
    sl.Values['sti1'] := '-1';
    sl.Values['tag'] := '4';
...
function TForm1.findndofbidloc(bid: integer): pvirtualnode;
var
  i: integer;
  nd: pvirtualnode;
  Data: pvstrecordloc;
begin
  ...
  try
    ...
    on e: Exception do
    begin
      form1.log('e', 'findndofbidloc,e=' + e.message + '/i=' + IntToStr(i));
    end;
  end;
end;
...
function TForm1.findndofbid(bid: integer): pvirtualnode;
var
  i: integer;
  nd: pvirtualnode;
  Data: pvstrecord;
begin
  ...
  try
    ...
    on e: Exception do
    begin
      form1.log('e', 'findndofbid,e=' + e.message + '/i=' + IntToStr(i));
    end;
  end;
end;
...
function tform1.fnnaloc(cnd:pvirtualnode;dsn:tnewds):pvirtualnode;
var
 nd:pvirtualnode;
 dsx:pvstrecordloc ;
begin
    nd:=vstloc.AddChild(cnd,nil);
    //nd:=vstloc.InsertNode(cnd,amInsertAfter,nil);
    dsx:=vstloc.getnodedata(nd);
    if dsn.ndcheck_state then begin
          ...
          Dsx := VSTloc.GetNodeData(nd);
          if not (vsInitialized in nd^.States) then  VSTloc.ReinitNode(nd, False);
        end;

       dsx^.dbmyid:=dsn.dbmyid;
       dsx^.bp:=dsn.bp;
       dsx^.nm0:=dsn.nm0;
       dsx^.nm1:=dsn.nm1;
       dsx^.tag:=dsn.tag;
...
function tform1.fnniloc(cnd:pvirtualnode;dsn:tnewds):pvirtualnode;
var
 nd:pvirtualnode;
 dsx:pvstrecordloc ;
begin
...
begin
    ...
    //  showmessage('prolog2 start');
      ...
      sl.text:=readconfig;
     // showmessage('prolog2 sl='+sl.text);
      umain.mysysinfo.baseaddr:=sl.Values['lebaseip'];
      umain.mysysinfo.basename:=sl.Values['lebasenane'];
      umain.mysysinfo.basepsw :=sl.Values['lebasepsw'];
      umain.mysysinfo.traddr  :=sl.Values['letransip'];


      form1.log('c','umain.mysysinfo.baseaddr='+umain.mysysinfo.baseaddr);
      log('c','umain.mysysinfo.basename='+umain.mysysinfo.basename);
      log('c','umain.mysysinfo.baspsw ='+umain.mysysinfo.basepsw);
    //  showmessage('call prolog3');
      prolog3;
...
begin
         ...
         // showmessage('ts='+ts);
          ...
          s:='APP='+appexe +' VERSION AT='+' '+APPDIR+app+' ## '+ts;
          caption:=s;
...
var
  D,FileDateTime,S,cn,ts:STRING;
   DataInf   : TSearchRec;
  i,size:integer;
  buffer: array[0..255] of char;
begin;
 //showmessage('start prolog1');
TIMERSTART.Enabled:=FALSE;
...
{$ENDIF}
    ...
    //  showmessage('next 1');
     ...
     appDir     := ExtractFilePath(FORMS.Application.ExeName);
     appExe     := ExtractFileName(FORMS.Application.ExeName);
     app        := ualfunc.ExtractStr(1,appdir,'.');
...
procedure TForm1.vstDblClick(Sender: TObject);
  ...
  begin
    ...
    if d^.tag =  2 then begin
     //showmessage('call editac');
    formvst2.left:=30;
    formvst2.top:=30;
    formvst2.BringToFront;
    formvst2.visible:=true;
    formvst2.editac(currnd);
    end;
      if d^.tag =  0 then formeditcomp.editcomp(d);
    if d^.tag =  1 then
     begin
      formeditch.BringToFront;
      formeditch.Visible:=true;formeditch.editch(d);
     end;
...
begin
   CURRNDLOC := NODE;
    vstloc.PopupMenu :=NIL;
  currmtag := -1;
  dataloc := vstLOC.GetNodeData(node);

  if dataloc = nil then begin
    vstloc.PopupMenu := popLOCNIL;
    log('y', 'NNNNNNNNNNNNNNNNNNNNNIL');
   exit;
  end;
  CURRNDLOC := NODE;
  dataloc := vstLOC.GetNodeData(node);
  ...
  log('w','BID='+inttostr(dataloc^.dbmyid)+zp+'tag=' + IntToStr(dataloc^.tag)+zp+'nm0='+dataloc^.nm0);
  ...
  begin
    vstloc.PopupMenu:=poploc3;
    exit;

  end;
   if dataloc^.tag = 21 then
  begin
    vstloc.PopupMenu:=poploc21;
  ...
  begin
   ...
   if (dataloc^.tag = -5) and ( dataloc^.sens='key') then
  begin
     vstloc.PopupMenu:=xpoplocm5;
  end;
...
begin
  ...
  if dl^.tag = 0 then form1.locread0;
  log('l', 'dbl ??????????????????????????????????');
...
function  TForm1.gettempera:tstringlist;
var
sl:tstringlist;
cmd,pt:string;
...
begin
      ...
      sl:=tstringlist.Create;
      pt:=appdir+'tempera.txt';
      pt:='/mnt/ramdisk/'+'tempera.txt';
      log('l',pt);
      cmd:='sensors>'+pt;
...
begin
     ...
     jscmd.cmd:='ac_genkeys';
     jscmd.typ:='genk';
     jscmd.k1:='1';
     jscmd.kn:='1000';
     form1.frmjs_ac(currnd,jscmd);
end;
...
procedure tform1.frmjs_ac(cnd:pvirtualnode;jscmd:tjscnd);
var
  jb:TJSONObject;
  jd:tjsondata;
  ch,sj,s,comp,bidch,bpp,bidac,bpsens,evpid,sens,st,kluch,code,mrs,NTMZ:string;
  ds:pvstrecord;
  ac,bid,port,ev,pt,topic,cid:string;
begin
      ds:=vstloc.GetNodeData(cnd);
      ac:=ds^.NM0;
      ch:=ds^.ch;
      comp:=ds^.comp;
      bid :=inttostr(ds^.dbmyid);
      cid:=comp+'_'+ch ;
      log('y','FRM ac='+ac+'/ch='+ch+' /cid='+cid+'/comp='+comp);
      if jscmd.typ='ac' then begin
        jb:=TJSONObject.Create(['cmd',jscmd.cmd,
                           'bid'     ,bid,
                           'komu'    ,'maldms',
                            'ch'      ,ch,
                            'ac'      ,ac,
...

    {
     ...
     bidac:=inttostr(dl^.bidac);
      ...
      if sens='key' then begin
       KLUCH:='828E34';
       if not form1.Inputsg('Введите код ключа',kluch) then exit;
       kluch:=ualfunc.Azerol(kluch,12);
       log('c','kluch='+kluch);
     end;
...
procedure tform1.formirnewemul(cnd:pvirtualnode);
var
  jb:TJSONObject;
  jd:tjsondata;
  sj,s,bidch,bpp,bidac,bpsens,evpid,sens,st,kluch,code,mrs,NTMZ,cid,codekey,contuxt:string;
  dl:pvstrecordloc;
...
begin
     ...
     bidac:=inttostr(dl^.bidac);
      ...
      if sens='key' then begin
       ...
       if not form1.Inputsg('Введите код ключа',kluch) then exit;
       kluch:=ualfunc.Azerol(kluch,12);
       codekey:=inttostr(ualfunc.keytox(kluch));
       log('c','kluch='+kluch);
       contuxt:=inttostr(dateutils.DateTimeToUnix(now));
     end;
      jb:=TJSONObject.Create(['cmd','oneemul',
                            'komu'    ,'maldms',
                            'cid'    ,cid,
...
begin
     ...
     exit;
   {
     ...
     //  log('l','EMUL p05');
      Top:=0;
      log('l','EMUL p03 1');
      SHOWMESSAGE('EMUL 042');
      formemul.readpersons;
      SHOWMESSAGE('EMUL 043');
...
begin
      form1.mygrpx(currndloc);
end;

procedure TForm1.mygrpx(cnd:pvirtualnode);
var
s,nm,myid:string;
qrz :TSQLQuery;
dl:pvstrecordloc;
begin
     ...
     s:='select * from tssloc_locations where tag=3 and bp='+inttostr(dl^.dbmyid);
     log('y','mygrpx s='+s);
     qrz := TSQLQuery.Create(self);
     qrz.DataBase := formglink.pqc1;
     qrz.SQL.Add(s);
...
function  tform1.checklinkls(bidloc:string):integer;
var
s,v:string;
myid:integer;
begin
     result:=-1;
     s:='select myid from tss_sensors where bploc='+bidloc;
     log('y','checklinkls s='+s);
     v:=trim(form1.freadfld(s));
     if v='' then begin
      result:=-1;
      exit;

     end;
     log('y','checklinkls v='+v);
     try
      result:=strtoint(v);
   except
    on e: Exception do
    begin
      form1.log('e', 'checklinkls,e=' + e.message );
      result:=-1;
    end;
  end;
end;
...
function  tform1.checklinksl(bidsens:string):boolean;
var
s,v:string;
begin
     s:='select  bidsens  from tssloc_locations where myid='+bidsens;
     log('y','checklinksl s='+s);
     v:=form1.freadfld(s);
     log('y','checklinksl v='+v);
     if v=bidsens then result:=true;
...
var
s,bidloc,bidsens,rc1,rc2,nm0s,acport:string;
nd,ndloc:pvirtualnode;
d:pvstrecord;
dl:pvstrecordloc;
rcb:boolean;
n:integer;
bidport,bidch:string;  //sudabp
begin    // tss_locations
        // bpsens,bidcomp,bpsens,bidch
        ndloc:=currndloc;
        dl:=vstloc.GetNodeData(ndloc);
        bidloc:=inttostr(dl^.dbmyid);

        //bp:=inttostr(dl^.dbmyid);
        nd:=form1.findchckndtag(vst,4);

        if nd=nil then begin
         showmessage('НЕТ ОТМЕЧЕННОГО СЕНСОРА НА ЖЕЛЕЗЕ--> ОТМЕТЬТЕ  и ПОВТОРИТЕ');
         exit;
        end;
        d:=vst.GetNodeData(nd);
        bidport:=inttostr(d^.bidport);
        acport:=form1.getacport(bidport);
        log('r',' acport='+acport);

        nm0s:=d^.NM0;
        bidsens:=inttostr(d^.dbmyid);
        log('y','id='+inttostr(d^.dbmyid)+zp+'nm0='+d^.nm0);
        s:='update tssloc_locations set bidsens='+bidsens+zp+'bidport='+bidport+
        ' where myid='+bidloc;
        log('l','s='+s);
        rc1:=selfupd(s);

        s:='update tss_sensors set bploc='+bidloc+' where myid='+bidsens;
        log('y','s='+s);
        rc2:=selfupd(s);
        log('y','rc1='+rc1+'/rc2='+rc2);
        n:=checklinkls(bidloc);
        d^.sti:=42;
        vst.refresh;
        if (rc1='ok') and (rc2='ok') and (rcb=true)  then begin
         dl^.sti:=42;
         vstloc.Refresh;
        end;
...
begin
     ...
     log('r','s='+s);
     form1.selfupd(s);
     s:='delete  from tssloc_locations where tag=3 and bp='+inttostr(dl^.dbmyid);
     log('r','s='+s);
     form1.selfupd(s);
     locread0;
...
procedure TForm1.locread3(ndp:pvirtualnode);
var
qrx: TSQLQuery;
s,nm,ttg,dbmyid,ng,bp,bidsens,acp,ac,bidport:string;
newds:tnewds;
nd:pvirtualnode;
ds:pvstrecordloc;
myid:integer;
begin
  ...
  if cbloc1.Checked then exit;

  ds:=vstloc.GetNodeData(ndp);
  dbmyid:=inttostr(ds^.dbmyid);
  s := 'select * from tssloc_locations  where tag=3 and bp='+dbmyid+' order by myid' ;
  qrx := TSQLQuery.Create(self);
  qrx.DataBase := formglink.pqc1;
  qrx.SQL.Add(s);
  qrx.Active := True;
  while not qrx.EOF do begin
   nm:=qrx.FieldByName('name').asstring;
   ttg:=qrx.FieldByName('tag').asstring;
   dbmyid:=qrx.FieldByName('myid').asstring;
   ng:=qrx.FieldByName('ng').asstring;
   bp:=qrx.FieldByName('bp').asstring;
   bidport:=qrx.FieldByName('bidport').asstring;
   bidsens:=qrx.FieldByName('bidsens').asstring;
   log('l','dbmyid='+dbmyid);
   newds.nm0:=nm;
   newds.tag:=strtoint(ttg);
   myid:=checklinkls(dbmyid);
  // showmessage('myid='+inttostr(myid));
   if myid>0 then    newds.sti:=42 else newds.sti:=44;
   newds.sti1:=-1;
   newds.ndcheck:=false;
   newds.ndcheck_state:=false;
   acp:=form1.getacport(bidport);
   //showmessage('acp='+acp);
   newds.NM1:='id='+dbmyid+zp+'acp='+acp+zp+'ng='+ng+zp+'bs='+bidsens;
   newds.dbmyid:=strtoint(dbmyid);
   nd:=form1.fnnaloc(ndp,newds);
   ds:=vstloc.GetNodeData(nd);
...
procedure TForm1.locread21(ndp:pvirtualnode);
var
qrx: TSQLQuery;
s,nm,ttg,dbmyid,mng,myid,bp:string;
newds:tnewds;
nd:pvirtualnode;
ds:pvstrecordloc;
begin
  ...
  s := 'select * from tssloc_locations  where tag=21 and bp='+dbmyid+' order by myid' ;
  qrx := TSQLQuery.Create(self);
  qrx.DataBase := formglink.pqc1;
  qrx.SQL.Add(s);
  qrx.Active := True;
  ...
  while not qrx.EOF do begin
   nm:=qrx.FieldByName('name').asstring;
   myid:=qrx.FieldByName('myid').asstring;

   //showmessage('read21='+nm);
   ttg:=qrx.FieldByName('tag').asstring;
   dbmyid:=qrx.FieldByName('myid').asstring;

  {
   s:='select max(ng) from tssloc_locations where '+
      ' tag=3 and bp='+dbmyid;
   mng:=form1.freadfld(s);
   }

   newds.nm0:=nm+'---';
   newds.dbmyid:=strtoint(myid);
   newds.tag:=strtoint(ttg);
   newds.sti:=-1;
   newds.sti1:=-1;
   newds.NM1:='id='+myid+zp+zp+'bp='+bp;
   newds.ndcheck:=false;
   newds.ndcheck_state:=false;
   newds.dbmyid:=strtoint(dbmyid);
   newds.bp:=strtoint(dbmyid);
   nd:=form1.fnnaloc(ndp,newds);
   ds:=vstloc.GetNodeData(nd);
   //locread21(nd);
   locread3(nd);
...
function tform1.calcng(dbmyid:string):integer;
var
s:string;
begin
   s:=' select count(*) from tssloc_locations where  tag=21 and bp='+dbmyid;
   result:=strtoint(form1.freadfld(s));


end;

procedure TForm1.locread2x(ndp:pvirtualnode);
var
qrx: TSQLQuery;
s,nm,ttg,dbmyid,mng,myid,bp:string;
newds:tnewds;
...
begin
  ...
  s := 'select * from tssloc_locations  where tag=2 and bp='+dbmyid+' order by myid' ;
  qrx := TSQLQuery.Create(self);
  qrx.DataBase := formglink.pqc1;
  qrx.SQL.Add(s);
  qrx.Active := True;
  ...
  // showmessage('read2='+nm);
   ttg:=qrx.FieldByName('tag').asstring;
   dbmyid:=qrx.FieldByName('myid').asstring;

   newds.nm0:=nm;
   mng:=inttostr(calcng(dbmyid));
   if mng='?' then exit;
   ...
   newds.sti:=-1;
   newds.sti1:=-1;
   newds.NM1:='id='+myid+zp+'ng='+mng+zp+'bp='+bp;
   newds.ndcheck:=false;
   newds.ndcheck_state:=false;
   newds.dbmyid:=strtoint(dbmyid);
   newds.bp:=strtoint(dbmyid);
...
procedure TForm1.locread2(ndp:pvirtualnode);
var
qrx: TSQLQuery;
s,nm,ttg,dbmyid,mng,myid,bp:string;
newds:tnewds;
...
procedure TForm1.locread1(ndp:pvirtualnode);
var
qrx: TSQLQuery;
s,nm,ttg,dbmyid:string;
newds:tnewds;
nd:pvirtualnode;
ds:pvstrecordloc;
bp:integer;
begin
  ...
  bp:=ds^.bP;
  dbmyid:=inttostr(ds^.dbmyid);
  s := 'select * from tssloc_locations  where tag=1 and bp='+dbmyid+' order by myid' ;
  qrx := TSQLQuery.Create(self);
  qrx.DataBase := formglink.pqc1;
  qrx.SQL.Add(s);
  qrx.Active := True;
...
begin
      vstloc.Clear;
  s := 'select * from tssloc_locations  where tag=0 order by myid' ;
  qrx := TSQLQuery.Create(self);
  qrx.DataBase := formglink.pqc1;
  qrx.SQL.Add(s);
  qrx.Active := True;
...
s:string;
d:pvstrecord;
ac,bidch,tscd,info:string;

jscmd:tjscnd;
begin

     jscmd.cmd:='ac_getinfo';
     jscmd.typ:='ac';
     form1.frmjs_ac(currnd,jscmd);
     ualfunc.MyDelay(2000);

       d:=vst.GetNodeData(currnd);
       bidch:=inttostr(d^.bidch);
       ac:=d^.nm0;
       s:='select jsinfo from tss_acinfo  where ac='+ac+
           ' and bpch='+bidch+' order by myid desc limit 1';

       info:=form1.freadfld(s);

        s:='select tscd from tss_acinfo  where ac='+ac+
           ' and bpch='+bidch+' order by myid desc limit 1';

       tscd:=form1.freadfld(s);
       s:=tscd+zp+info;
...
jscmd:tjscnd;
begin
     //showmessage('read direct');
     jscmd.cmd:='ac_readallkeys';
     jscmd.typ:='ac';
     form1.frmjs_ac(currnd,jscmd);
end;
...
d:pvstrecord;
dl:pvstrecordloc;
ac,bidport,cid,port,sens:string;
begin
    log('y','EMULATOR');
     nd:=findchckndtag(vst,3);
     if nd=nil  then begin
      showmessage('отмечено более одгого порта или ни одного' );
      exit;
     end;
     dl:=vstloc.GetNodeData(currndloc);
     d:=vst.GetNodeData(nd);
     ...
     vemuldata.ch:=d^.ch;
     vemuldata.cid:=cid;
     vemuldata.port:=d^.nm0;
     vemuldata.sens:=ualfunc.extractstr(1,dl^.nm0,'_');
     log('y','sens='+vemuldata.sens+',port='+vemuldata.port);
...
begin    // tss_locations
        ...
        if nd=nil then begin
         showmessage('НЕТ ОТМЕЧЕННОГО ПОРТА НА ЖЕЛЕЗЕ ИЛИ ОТМЕЧЕНО БОЛЕЕ ОДНОГО --> ОТМЕТЬТЕ  и ПОВТОРИТЕ');
         exit;
        end;
        plinksgrponport(nd,currndloc);
...
begin
      ...
      s:='update tss_sensors set bploc=-1 where tag=4 and  bp='+myid;
      selfupd(s);
      s:='update    tssloc_locations set bidsens=-1'+
      ' where bidport='+myid;
      selfupd(s);
      vst.refresh;
      vstloc.Refresh;
...
procedure tform1.plinksgrponport(ndport,cndl:pvirtualnode);
var
dl:pvstrecordloc;
d :pvstrecord;
bidport,bidgrp,myid:string;
qrz: TSQLQuery;
code,s: string;
aid :array[1..4] of string;
aids:array[1..4] of string;
n:integer;
nd:pvirtualnode;


begin
        d:=vst.GetNodeData(ndport);
        dl:=vstloc.GetNodeData(cndl);
        bidport:=inttostr(d^.dbmyid);
        bidgrp:=inttostr(dl^.dbmyid);
        log('y','bidport='+bidport+',bidgrp='+bidgrp);
        s := 'select *from  tssloc_locations where tag=3 and bp='+bidgrp+' order by code';
        qrz := TSQLQuery.Create(self);
        qrz.DataBase := formglink.pqc1;
        qrz.SQL.Clear;
        qrz.SQL.Add(s);
        ...
        while not qrz.EOF do begin
         ...
         n:=strtoint(code);
         aid[n]:=myid;
         s:='update tssloc_locations set bidport='+bidport+
         ' where myid='+myid;
         log('y','s='+s+',code='+code);
         selfupd(s);
         nd:=findndofbidloc(strtoint(myid));
        ...
        qrz.Close;
        qrz.Free;
        s := 'select * from  tss_sensors where bp='+bidport+' order by code';
        qrz := TSQLQuery.Create(self);
        qrz.DataBase := formglink.pqc1;
        qrz.SQL.Clear;
        qrz.SQL.Add(s);
        qrz.Active:=true;
          while not qrz.EOF do begin
            ...
            n:=strtoint(code);
            aids[n]:=myid;
            s:='update tss_sensors set bploc='+aid[n]+
            ' where myid='+myid;
            selfupd(s);
            log('w','s='+s+',code='+code);
            nd:=findndofbid(strtoint(myid));
            d:=vst.GetNodeData(nd);
            d^.sti:=42;
            qrz.next;
          end;
        ...
        while not qrz.EOF do begin
         ...
         code:=qrz.FieldByName('code').AsString;
         n:=strtoint(code);
         s:='update tssloc_locations set bidsens='+aids[n]+' where myid='+myid;
         selfupd(s);
         qrz.next;
        end;
...
var
  ...
  begin
   ...
   //showmessage('call formvst2');
   formvst2.Top:=0;
   formvst2.left:=0;
   formvst2.visible:=true;
   formvst2.BringToFront;
   formvst2.addac(currnd);
    //showmessage('exit formvst2');
...
procedure TForm1.m1107add(ttg:string);
var
    s,nm:string;
begin
       nm:='tag-'+ttg;
...
begin
         if not form1.Inputsg('название ???',nm) then exit;
         newds.nm0:=nm;
         newds.sti:=-1;
         newds.sti1:=-1;
         newds.tag:=0;
...
begin
  s := Value;
  Result := InputQuery('название ??? ', Prompt, s);
  if Result then
    Value := s;
  //Value :=LowerCase (s);
end;
...
begin
       dl:=vstloc.GetNodeData(currndloc);
       if not form1.Inputsg('ВВЕДИТЕ НАЗВАНИЕ подобъекта',NME) then exit;
...
begin
        ...
        //if yesno(
        //   '@@@@ Вы действительно хотите покиеуть приложение') then begin
        halt(0);
...
begin
   if yesno('ВЫ действительно хотите добавить стандартнуд группу сенсоров ???') then begin
    //formalnewlocsens.aladdgrp(currndloc);
     m711addgrp(currndloc);
   end;
end;

procedure tform1.m711addgrp(cnd:pvirtualnode);
var
  i,ng:integer;
  dbmyid,s,nm,bp:string;
  ds:pvstrecordloc;
  qrx :TSQLQuery;
begin
      ds:=vstloc.GetNodedata(cnd);
      dbmyid:=inttostr(ds^.dbmyid);
      s:='select count(*) from tssloc_locations where '+
      ' tag=21 and bp='+dbmyid;
      log('r','s='+s);
      ...
      qrx.Active := True;
      ng:=qrx.Fields[0].asinteger;
      log('y','ng='+inttostr(ng));
      ng:=ng+1;
      log('r','ng='+inttostr(ng));

      SHOWMESSAGE('tut NG='+inttostr(ng));
      nm:=ds^.nm0+' группа='+inttostr(ng);
      s:='insert into tssloc_locations(ng,bp,tag,name)values('+
      inttostr(ng)+zp+
      dbmyid+zp+
      '21'+zp+
      ap+nm+ap+')';
      ...
      s:='select myid from tssloc_locations  order by myid desc limit 1' ;
      bp:=freadfld(s);
      log('l','s='+s);
      showmessage('bp='+bp);
      //exit;
      ...
      for i:=1 to 4 do begin
       if i=1 then nm:='key';
       if i=2 then nm:='open';
       if i=3 then nm:='close';
       if i=4 then nm:='rte';
       nm:=nm+'_'+inttostr(ng) ;

       s:='insert into tssloc_locations(ng,tag,bp,code,name)values('+
       inttostr(ng)+zp+
       '3'+zp+
       bp+zp+
       inttostr(i)+zp+
...
procedure tform1.readinsidegrp(cnd:pvirtualnode;bp:string);
var
s,nm,myid,code,ngs:string;
qrz: TSQLQuery;
newds:tnewds;
nd:pvirtualnode;
begin
    s:='select myid,name,tag,ng,code,bp  from tssloc_locations '+
    ' where tag=3  and bp='+bp+
    ' order by ng,code asc ';
     qrz := TSQLQuery.Create(self);
     qrz.DataBase := formglink.pqc1;
     qrz.SQL.Add(s);
     qrz.Active := True;
   //  log('r','s='+s);
     ...
     while not qrz.eof do begin
       nm:=qrz.FieldByName('name').AsString;
       ngs:=qrz.FieldByName('ng').AsString;

       myid:=qrz.FieldByName('myid').AsString;
       code:=qrz.FieldByName('code').AsString;
       bp  :=qrz.FieldByName('bp').AsString;
      {
       if strtoint(code)=1 then begin
         newds.nm0   :=nm;
         newds.sti:=-1;
         newds.sti1:=-1;
         newds.tag:=21;
         newds.dbmyid:=strtoint(myid);
         newds.Code:=strtoint(code);

         newds.ndcheck:=false;
         newds.ndcheck_state:=false;
         newds.NM1:='myid='+myid+zp+'ng='+ngs+zp+'bp='+bp;
         nd:=form1.fnniloc(cnd,newds);
       end;
       }
...
begin
     ...
     dl:=vstloc.GetNodeData(cnd);
     s:='select myid from tssloc_locations where tag=21 and bp='+inttostr(dl^.dbmyid);
     log('y','s='+s) ;
     bp:=freadfld(s);
     log('y','bp='+bp) ;
     s:='select * from tssloc_locations where tag=21 and bp='+inttostr(dl^.dbmyid);
     qrz := TSQLQuery.Create(self);
     qrz.DataBase := formglink.pqc1;
     qrz.SQL.Add(s);
     qrz.Active := True;
...
var
dl:pvstrecordloc;
dbmyid,bP,bpp:string;
begin
     dl:=vstloc.GetNodeData(currndloc);
     bpp:=inttostr(dl^.bpp);
   //  showmessage('readingrp MenuItem45Click bpp='+bpp);
     dbmyid:=inttostr(dl^.dbmyid);
     bp    :=inttostr(dl^.bp);
     log('c','CALL INGRP dbmyid='+dbmyid+' / bp='+bp);
     // showmessage('readingrp call readingrp');
     readingrp(currndLOC,'');
end;
...
begin
       ...
       log('y','getchonbpsens myid='+inttostr(myid));
       exit;
...
begin
      ...
      showmessage('64c 1');
       formlocm5.Top:=0;
       formlocm5.left:=0;
       formlocm5.Visible:=true;
       formlocm5.BringToFront;
...
function tform1.faquass(ssmyid:string):boolean;
var
bploc,s:string;
d:pvstrecord;
begin
         s:='select ss.bploc,lc.myid from'+
         ' tss_sensors  as ss'+zp+
         ' tssloc_locations as lc'+
         ' where ss.myid='+ssmyid+
         ' and ss.bploc=lc.myid';
         log('l','faquass s='+s);
         bploc:=form1.freadfld(s);
         log('y','bploc='+bploc);
...
begin
      ...
      rc:=faquass(myid);
      if rc then begin
       log('l','TTTTTTTTTTTTTTTTTTTTTTTTTTTTTT');
...
begin
      d:=vst.GetNodeData(currnd);
      myid:=inttostr(d^.dbmyid);
      s:='select bploc from tss_sensors where myid='+myid;
      bploc:=form1.freadfld(s);
      s:='update tss_sensors set bploc=-1 where myid='+myid;
      log('y','s='+s);
      selfupd(s);
      s:='update  tssloc_locations set bidsens=-1,bidport=-1'+
      ' where myid='+bploc+' and tag=3';
      log('y','s='+s);
      selfupd(s);
      vst.refresh;
      vstloc.Refresh;
...
function tform1.updsensbploc(bploc,bidport,code:string):string;
var
 bidsens:string;
 s:ansistring;
...
begin
         ...
         s:='update tss_sensors set bploc='+bploc+' where bp='+bidport+
         ' and code='+code;

         log('c','updsensbploc 1='+s);
         form1.selfupd(s);
         s:='select myid from tss_sensors where bploc='+bploc+' and bp='+bidport+
         ' and code='+code;
         log('c','updsensbploc 2='+s);
         result:=form1.freadfld(s);
...
function tform1.fnewlinks(ndp,cnd:pvirtualnode):boolean;  //suda1102
var
 s,bidport,mpcl,code,nm,bidloc,bpl,bidsens:string;
 ds:pvstrecord;
 dl,dsp:pvstrecordloc;
 qrx:tsqlquery;
 ndport:pvirtualnode;
 n:integer;
 begin
       ...
       n:=0;
       dl:=vstloc.GetNodeData(cnd) ;
       mpcl:=inttostr(dl^.mpcl);
       ndport:=dl^.ndp;
       bpl:=inttostr(dl^.bp);
       dsp:=vstloc.GetNodeData(ndport);
       //showmessage(dsp^.nm0+',bpl='+bpl) ;
       //exit;
       ds:=vst.GetNodeData(ndp);
       bidport:=inttostr(ds^.dbmyid);
      ...
      ' and mpcl='+mpcl;
       ...
       qrx:=formglink.gselqr(s);
       log('l','==============================================================') ;
      // showmessage('fnewlinks rc='+inttostr(qrx.RecordCount));
       while not qrx.eof do begin
        nm:=qrx.FieldByName('name').asstring;
        code:=qrx.FieldByName('code').asstring;
        bidloc:=qrx.FieldByName('myid').asstring;
        log('l','n='+inttostr(n)+',nm='+nm+zp+'bidloc='+bidloc);
        bidsens:=updsensbploc(bidloc,bidport,code);
        // showmessage('fnewlinks bidsens='+bidsens);
        if bidsens <>'?'then begin
         s:='update tssloc_locations set bidsens='+bidsens+zp+'bidport='+bidport+
         ' where myid='+bidloc+' and mpcl='+mpcl;
         log('l','s============='+s);
         form1.selfupd(s);
  ...
  {
       s:=' select myid,name,code,mpcl,bp from tssloc_locations '+
       ' where'+
       ' bidport='+bidport+
       ' and tag=3'+
       ' and mpcl='+mpcl;
...
begin
        ...
        dl:=vstloc.GetNodeData(currndloc);
        sl:=tstringlist.Create;
        ndp:=form1.findchckndtag(vst,3);
        if ndp=nil then begin
         showmessage('НЕ НАЙДЕН ОТМЕЧЕННЫЙ ПОРТ--> ОТМЕТЬТЕ  и ПОВТОРИТЕ');
         exit;
        end;
        fnewlinks(ndp,currndloc);
        reread;
        rereadloc;
...
procedure TForm1.pcmdprgs(sl:tstringlist);
var
 nd:pvirtualnode;
 d:pvstrecord;
 bid:integer;
...
begin
        ct:=timetostr(time);
        log('l','pcmdprgs='+sl.text);
        bid:=strtoint(sl.Values['bid']);
        nd:=form1.findndofbid(bid);
        d:=vst.GetNodeData(nd);
        if sl.Values['phase']='wait' then begin
         d^.NM1:=ct+' Получена '+sl.values['subcmd']+zp+'phase='+sl.Values['phase']+zp+'t='+sl.Values['secmer'];
          d^.sti:=43;
        end;
        if sl.Values['phase']='exe' then begin
         d^.NM1:=ct+' Выполняется  '+sl.values['subcmd']+zp+'phase='+sl.Values['phase']+zp+
         ' # '+sl.Values['attempt']+zp;
           d^.sti:=40;
          end;

        if sl.Values['phase']='endok' then begin
         d^.NM1:=ct+' Завершено  '+sl.values['subcmd']+zp+'err='+sl.Values['erm'];
          d^.sti:=42;
        end;
        if sl.Values['phase']='enderr' then begin
          d^.NM1:=ct+' Завершено  '+sl.values['subcmd']+zp+'err='+sl.Values['erm'];
           d^.sti:=44;
        end;
...
begin
       // showmessage('razborinjs='+v);
        ...
        // log('r','NEW cmd      ='+cmd+'      /SUBCMD='+subcmd+' /fn='+fn);
         ...
         cmd:=sl.values['cmd'];
         subcmd:=sl.Values['subcmd'];
         if cmd='cmd_progress' then begin
           pcmdprgs(sl);
         end;
...
begin
  ...
  log('c','bidport='+inttostr(data^.bidport)+zp+
  'ch='+data^.ch+zp+'tag='+inttostr(data^.tag)+zp+'nm0='+data^.nm0+zp+'myid='+inttostr(data^.dbmyid)+zp+'bidch='+inttostr(data^.bidch)+zp+'ac='+data^.ac+zp+'bidac='+inttostr(data^.bidac));
 // datandp:=vst.GetNodeData(ndp);
  if data^.tag=-1 then begin
   log('y','popminus');
