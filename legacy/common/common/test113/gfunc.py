...
def allsmasktostr(ls):
    s = ''
    for x in ls:
        s = s + str(x)
    return s

def alfdecmaska(maska):
    p=0
    for n in maska:
        n = int(n)
        if n > 0:
...
def alfsetbit(value, bit):
    return value | (1<<bit)

def alfgetbit(num, pos):
...
def al_209dt(ac,dt):
  try:
    g={}
    g['ac']=ac
    g['year']=str(dt.year+2000)
    g['month'] = str(dt.month)
    g['day']   = str(dt.day)
    g['hour'] = str(dt.hour)
    g['minute'] =str(dt.minute)
    g['second'] =str(dt.second)
    #dt['hour'] = zerol(dt['hour'], 2)
    g['hour']  = str(dt.hour)
    #dt['minute']=zerol(dt['minute'],2)
    g['minute']= dt.minute
    #dt['second'] = zerol(dt['second'], 2)
    g['second']= str(dt.second)
    dt = str(g['year']) + '-' + str(g['month']) + '-' + str(g['day']) + ' ' + \
         str(g['hour']) + ':' + str(g['minute']) + ':' + str(g['second'])
    uxt = stofunc.datetimetosec(dt, 'sql')
    n = stofunc.nowtosec('loc')
    #print ('dt=',dt)
    d = n - uxt
    g['uxt']=uxt
    g['dt']=dt
...
def readstarter(vb):
    try:
        ...
        for row in vb.cursor:
            ...
            g[app]={}
            tag = str(row['tag'])
            adrpath = row['adrpath'].strip()
            dbn = row['dbn'].strip()
            print ('dbn=',dbn)
            psw = row['psw'].strip()
            print('psw=', psw)
            print ('FIRST dbn=',dbn,'adrpath=','adrpath','tag=',tag,'psw=',psw)
            #if app=='base':
            ...
            mpv('magenta','app='+app+',adrpath='+adrpath+',dbn='+dbn+' psw=' + psw)
        return g
    except Exception as ee:
     mpv('readstarter', ee)
...
def almymarker():
    ...
    ls=t1.split(' ')
    tx1=ls[1]
    #mp('yellow','tx1='+str(tx1))
    ls=tx1.split(':')
    tx2=ls[2]
   # mp('yellow','tx2='+str(tx2))
    rc=str(float(tx2))
    return rc
