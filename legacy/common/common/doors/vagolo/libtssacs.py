...
class Channel(object):
    ...
    def _read_until(self, size, *any_of):
        ...
        #print(f'libtssacs._read_until -> BEGIN => size ({type(size)}): {size};   any_of ({type(any_of)}): {any_of}')
    
        assert size > 0
        r = bytearray()
        n = size

        #print(f'libtssacs._read_until -> DO while True...')

        while True:
            r += self._read_some(n)
            #print(f'   [{len(r)}] r ({type(r)}): {r}') # !!! NO
        ...
        # FT:
        #print(f'libtssacs._read_until -> n ({type(n)}): {n};   r ({type(r)}): {r}')
        #if len(r) != 0:
        #    print(f'libtssacs._read_until -> n ({type(n)}): {n};   r ({type(r)}): {r}')
    ...
    def find_addrs(self):
        ...
        for addr in range(0, 255):
            q[2] = addr
            with self._lock:
                #print(f"libtssacs.Channel.find_addrs -> q ({type(q)}): {q}")
                #q (<class 'bytearray'>): bytearray(b'\x16 \x00')
                #self._write(q)
                try:
                    self._write(q)
                except Exception as E:
                    print("libtssacs.Channel.find_addrs -> ERROR:",E)
    ...
    def read_all_keys(self, addr):
        ...
        for block_idx in range(0, mi.unsorted_key_blocks):
            for buf_idx in range(0, bufs_per_block):
                ...
                start = 0
                if buf_idx == 0:
                    rec_num = buf[7] if buf.startswith(b'DALLAS') != -1 else 0
                    if rec_num == 0:
                        _raise_failed()
                    start = 1
        ...
        while keys_done != len(keys):
            for buf_idx in range(0, bufs_per_block):
                if keys_done != len(keys):
                    ...
                    #print('libtssacs.write_all_keys -> sleep 1sec...')
                    #gevent.sleep(1) # VVG
    ...
    def _write_to_ram(self, addr, mem_bank, mi, offset, data):
        #print(f'libtssacs._write_to_ram -> BEGIN...')
        #print(f'libtssacs._write_to_ram -> BEGIN => addr ({type(addr)}): {addr};   mem_bank ({type(mem_bank)}): {mem_bank};   mi ({type(mi)}): {mi};   offset ({type(offset)}): {offset};   data ({type(data)}): {data};')
        #print(f'libtssacs._write_to_ram ->\n   mem_bank ({type(mem_bank)}): {mem_bank};\n   mi.bank(offset) ({type(mi.bank(offset))}): {mi.bank(offset)};')

        if mem_bank != mi.bank(offset):
    
            #print(f'libtssacs._write_to_ram -> 1. sleep 1sec')
            #gevent.sleep(1)
                
            self._cmd_84(addr, MEM_TYPE_RAM, mem_bank, MEM_OP_OFF, 0)

            #print(f'libtssacs._write_to_ram -> 2. sleep 1sec')
            #gevent.sleep(1)

            mem_bank = mi.bank(offset)

            #print(f'libtssacs._write_to_ram -> 3. sleep 1sec')
            #gevent.sleep(1)

            self._cmd_84(addr, MEM_TYPE_RAM, mem_bank, MEM_OP_WR, 0)

            #print(f'libtssacs._write_to_ram -> 4. sleep 1sec')
            #gevent.sleep(1)

        #print(f'libtssacs._write_to_ram -> 5. sleep 1sec')
        #gevent.sleep(1)

        self._cmd_dx(addr, mi.bank(offset), offset, data)

        #print(f'libtssacs._write_to_ram -> 6. sleep 1sec')
        #gevent.sleep(1)

        offset += mi.wr_buf_size
        
        # FT:
        #print(f'libtssacs._write_to_ram -> END: mem_bank ({type(mem_bank)}): {mem_bank};   offset ({type(offset)}): {offset};')
        #if len(r) != 0:
        #    print(f'libtssacs._write_to_ram -> n ({type(n)}): {n};   r ({type(r)}): {r}')
    ...
    def _ensure_responded(self, response):
        ...
        # FT:
        #if len(response) != 0:
        #    print(f'libtssacs._ensure_responded -> response ({type(response)}): {response}')
        #    #gevent.sleep(1) # VVG
    ...
    def _cmd_dx(self, addr, mem_bank, offset, data):
        ...
        #print(f'libtssacs._cmd_dx -> addr ({type(addr)}): {addr};   mem_bank ({type(mem_bank)}): {mem_bank};   offset ({type(offset)}): {offset};   data ({type(data)}): {data}')
        #addr (<class 'int'>): 5;   
        #mem_bank (<class 'int'>): 0;   
        #offset (<class 'int'>): 4096;   
        #data (<class 'bytearray'>): bytearray(b'DALLAS\x00\n\x02\x00\x00\x00\x00\x00\xff\x0f\x03\x00\x00\x00\x00\x00\xff\x0f\x04\x00\x00\x00\x00\x00\xff\x0f\x05\x00\x00\x00\x00\x00\xff\x0f\x06\x00\x00\x00\x00\x00\xff\x0f\x07\x00\x00\x00\x00\x00\xff\x0f\x08\x00\x00\x00\x00\x00\xff\x0f\t\x00\x00\x00\x00\x00\xff\x0f\n\x00\x00\x00\x00\x00\xff\x0f\x0b\x00\x00\x00\x00\x00\xff\x0f\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff')   
        
        
        q = bytearray([0x16, 0xF0 | mem_bank, addr, offset & 0xFF, (offset >> 8) & 0xFF, len(data)])
        #print(f'libtssacs._cmd_dx -> 111')

        q += data
        #print(f'libtssacs._cmd_dx -> 222')

        q.append(check_sum(data))
        #print(f'libtssacs._cmd_dx -> 333 q ({type(q)}): {q}') # !!! YES

        with self._lock:
            #print(f'libtssacs._cmd_dx -> 333-1') # !!! YES
            self._write(q)
            #print(f'libtssacs._cmd_dx -> 333-2') # !!! YES
            r = self._read_until(2, 0x16)
            #print(f'libtssacs._cmd_dx -> 333-3') # !!! NO
        #print(f'libtssacs._cmd_dx -> 444') # !!! NO

        b = r[1]
        #print(f'libtssacs._cmd_dx -> 555')

        if b == 0x15 or b == 0x1B or b == 4 or b == 0x18:
            #print(f'libtssacs._cmd_dx -> DO _raise_failed(r[1]) => r[1] ({type(r[1])}): {r[1]}')
            _raise_failed(r[1])
        #print(f'libtssacs._cmd_dx -> 666')

        if b != 6:
            #print(f'libtssacs._cmd_dx -> DO _raise_failed()')
            _raise_failed()
        #print(f'libtssacs._cmd_dx -> 777')
...
class ChannelRS232(Channel):
    ...
    def _read_some(self, size):
        ...
        #FT:
        if 1 < size:
            print(f'libtssacs._read_some -> BEFOERE "assert size > 0" => size ({type(size)}): {size};')
        
        #assert size > 0
        #assert size > 0, '"size" value is 0!!!'
        
        #print(f'libtssacs._read_some -> DO _port.read(size)...')
        ...
        try:
            ...
            if 1 < size:
                while True:
                    ...
                    print(f"   [{i}] len(r): {len(r)}")
                    r += self._port.read(size)
            #print(f'libtssacs._read_some -> DONE({i}) _port.read({size}) ??? len(r)={len(r)} => r ({type(r)}): {r}')
        except Exception as E:
            print(f"libtssacs._read_some -> Exception: {E.__class__}\n{str(E)}")

        # FT:
        if len(r) != 0 and 1 < size:
            print(f'libtssacs._read_some -> size ({type(size)}): {size};   r ({type(r)}): {r}')
...
class ChannelTCP(Channel):
    def __init__(self, host, port=5086, response_timeout=0.05):
        #print(f"libtssacs.ChannelTCP -> host ({type(host)}): {host};   port ({type(port)}): {port}")
        ...
        try:
            ...
            print("libtssacs.ChannelTCP.__init__ -> ERROR:",E)
    ...
    def _write(self, data):
        #print(f"libtssacs.ChannelTCP._write -> data ({type(data)}): {data}")
        #data (<class 'bytearray'>): bytearray(b'\x16 \x00')
        try:
            self._socket.sendall(data)
        except Exception as E:
            print("libtssacs.ChannelTCP._write -> ERROR:",E)
