...
class ChannelTCP(Channel):
    #def __init__(self, host, port=5086, response_timeout=0.05): # BAD for add_key in cycle [5000]
    #def __init__(self, host, port=5086, response_timeout=0.1): # BAD for add_key in cycle [9999]
    def __init__(self, host, port=5086, response_timeout=0.5): # OK for add_key in cycle [15000]
    #def __init__(self, host, port=5086, response_timeout=10):
    #def __init__(self, host, port=5086, response_timeout=1):
    ...
    def _read_some(self, size):
        ...
        try:
            ...
            print('ChannelTCP._read_some -> Except ('+str(type(E))+'->'+str(type(E).__name__)+'): '+str(E)+';   self.response_timeout: '+str(self.response_timeout)+';')
            raise ErrorNoResponse(self.response_timeout)
    # END: def _read_some-OLD(self, size):
