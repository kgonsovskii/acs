...
class Channel(object):
    ...
    def _read_until(self, size, *any_of):
        ...
        #print('_read_until -> any_of ('+str(type(any_of))+'): '+str(any_of)) # any_of (<class 'tuple'>): (171,)
        #print('_read_until -> size ('+str(type(size))+'): '+str(size)) # 3
        #print('_read_until -> n: '+str(n)) # 3

        while True:
            r += self._read_some(n)

            #print('_read_until -> len(r): ' + str(len(r)))  # 3
            #print('_read_until -> r.hex(): ' + r.hex()) # (hex) 414c4c => (str) 'ALL' => (dec) 65,76,76

            found = False
            for i, b in enumerate(r):

                #print('_read_until -> i ('+str(type(i))+'): ' + str(i)) # i (<class 'int'>): 0
                #print('_read_until -> b ('+str(type(b))+'): ' + str(b)) # b (<class 'int'>): 171
                ...
                #print('_read_until -> [in for] found ('+str(type(found))+'): ' + str(found)) #

            #print('_read_until -> [after for] found (' + str(type(found)) + '): ' + str(found))  #

            n = size - len(r)

            #print('_read_until -> n (' + str(type(n)) + '): ' + str(n))  # n (<class 'int'>): 0

            if n < 1:
                if found:
                    #print('_read_until -> while break!!!')
                    break

                #print('_read_until -> DO _raise_unexpected_response()')
                _raise_unexpected_response()

        #print('_read_until -> return r: ' + r.hex())  # (hex) 414c4c => (str) 'ALL' => (dec) 65,76,76
        return r
    ...
    def _events_or_keys_info(self, addr, op):
        ...
        #print('_events_or_keys_info -> addr: '+str(addr)+';   op: '+str(op))
        # _events_or_keys_info -> addr: 171;   op: 8

        r = self._cmd_4c(addr, op)
        #print('_events_or_keys_info -> r ('+str(type(r))+'): ')
        #print('_events_or_keys_info -> r ('+str(type(r))+'): '+str(r))
    ...
    def read_all_keys(self, addr):
        keys = []
        #print('read_all_keys -> addr: '+str(addr))
        ki = self.keys_info(addr) #(64000,100)
        #ki = (64000,10)
        if ki[1] == 0:
            return keys
    ...
    def _cmd_4c(self, addr, op):
        ...
        # print('_cmd_4c -> addr ('+str(type(addr))+'): ') # int
        # print('_cmd_4c -> op ('+str(type(op))+'): ') # int
        # print('_cmd_4c -> addr ('+str(type(addr))+'): ' + str(addr)+';   op ('+str(type(op))+'): '+str(op))
        # _cmd_4c -> addr( <class 'int'> ): 171;   op( <class 'int'> ): 8

        q = bytearray([0x16, 0x6C, addr, op])
        # print('_cmd_4c -> q ('+str(type(q))+'): '+q.hex()) # q (<class 'bytearray'>): 166cab08

        with self._lock:
            self._write(q)
            r = self._read_until(3, addr)

            #print('_cmd_4c -> type(r): '+str(type(r)))
            print('_cmd_4c -> r ('+str(type(r))+'): '+r.hex()) # r (<class 'bytearray'>): ab0702
            ...
            #print('_cmd_4c -> pack_len ('+str(type(pack_len))+'): '+str(pack_len))
            #print('_cmd_4c -> n ('+str(type(n))+'): '+str(n))
...
class ChannelTCP(Channel):
    ...
    def _read_some(self, size):
        if 1 < size:
            pass
            #print('libtssacs.ChannelTCP._read_some -> BEFORE "assert size > 0" => size ('+str(type(size))+'): '+str(size)+';')
...
#        #assert size > 0
        ...
        try:
           ...
           # gevent.sleep(0.01)
            ...
            #print('libtssacs.ChannelTCP._read_some -> len(r) ('+str(type(len(r)))+'): '+str(len(r))+';')
            #print('libtssacs.ChannelTCP._read_some -> r ('+str(type(r))+'): '+r.hex()+';')
        ...
        # except gevent.socket.timeout:
            ...
            #print('libtssacs.ChannelTCP._read_some -> Except: '+str(E)+';')
