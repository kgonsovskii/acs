...
type
  ...
  end; // TMQTTConnection = class(TThread)
...
begin
  ...
  {
  if IsConsole then WriteLn(msg)
  //else if assigned(OnLog) then OnLog(level, Msg);
  else if assigned(OnLog) then OnLog(MOSQ_LOG_ALL, Msg);
  }
...
begin
  ...
  if FMosquitto=Nil then raise Exception.Create('mosquitto instance creation failure');
    ...
    while not (Terminated and (State in [mqttDisconnected, mqttReconnecting, mqttNone ] )) do
      begin
        case State of
          ...
          mqttReconnecting: begin
              if ReconnectDelayExpired then Reconnect
              else
                Sleep(100);
            end;
          mqttConnected, mqttConnecting: mosquitto_loop(Fmosquitto, 100, 1);
        end;
      end;
    State:=mqttNone;
  except
  ...
  if assigned(Fmosquitto) then
    with FMosquitto do
      begin
        ...
        if assigned(OnPublish) then OnPublish(mid);
      end;
end;
  ...
  if assigned(FMosquitto) then
    with FMosquitto do
      begin
        ...
        State:=mqttConnected;
        if assigned(OnConnect) then OnConnect(rc);
      end;
end;
...
begin
  if assigned(FMosquitto) then
    with FMosquitto do begin
        Log(MOSQ_LOG_INFO,'Broker disconnected: '+disconnect_reason[rc = 0]);
        State:=mqttDisconnected;
        if assigned(OnDisconnect) then OnDisconnect(rc);
      end;
end;
...
begin
  ...
  if assigned(Fmosquitto) then
    with Fmosquitto do begin
        if (MOSQLogLevel and level) > 0 then begin
            writestr(tmp,'[MOSQUITTO] [',FName,'] ',mqtt_loglevel_to_str(level),' ',str);
            logger(tmp);
          //Log(MOSQ_LOG_ALL,str); // FT
        end;
        //if assigned(OnLog) then OnLog(level, str);
    end
  else
    begin
...
begin
  ...
  if not libinited then begin
      if verbose then begin
        ...
        //Log(MOSQ_LOG_ALL,'[MOSQUITTO] mosquitto init failed.'); // FT
      end;
      exit;
  end;
  ...
  if verbose then begin
    ...
    //Log(MOSQ_LOG_ALL,'[MQTT] Compiled against mosquitto header version '+IntToStr(LIBMOSQUITTO_MAJOR)+'.'+IntToStr(LIBMOSQUITTO_MINOR)+'.'+IntToStr(LIBMOSQUITTO_REVISION)); // FT
      logger('[MQTT] Running against libmosquitto version '+IntToStr(major)+'.'+IntToStr(minor)+'.'+IntToStr(revision));
    //Log(MOSQ_LOG_ALL,'[MQTT] Running against libmosquitto version '+IntToStr(major)+'.'+IntToStr(minor)+'.'+IntToStr(revision)); // FT
  end;
end;
...
initialization
  ...
  logger:=@mqtt_log;
finalization
  if libinited then mosquitto_lib_cleanup;
end.
