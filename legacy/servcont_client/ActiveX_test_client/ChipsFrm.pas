...
uses
  ...
  Dialogs, Grids, ComCtrls, StdCtrls, ExtCtrls, ActnList, Menus, BFServCont_TLB;
...
type
  ...
  public
    { Public declarations }
    Procedure Controls2Data(Ports : ITSSServCont_ControllerPorts;
...
Procedure TfrmChips.Controls2Data(Ports : ITSSServCont_ControllerPorts;
                           Chips : ITSSServCont_ControllerChipList);
var
  i, j: Integer;
  chb: TCheckBox;
  //Chip: TControllerChip;
  Chip : ITSSServCont_ControllerChip;
  V : ITSSServCont_ControllerKeyValue;
  S : String;
begin
  Chips.Count := Grid.RowCount - 2;
  if Chips.Count < 1 then
    Exit;
  ...
  for j:=0 to Ports.Count - 1 do begin
    ...
    Assert(chb <> nil);
    Ports.P[J] := chb.Checked;
  end;

  for i:=0 to Chips.Count - 1 do begin
    Chip := Chips.Items[I];
    //with Grid do begin
      //Move(Bin(Cells[0, i+1])[1], Value, cKeySize);
      Chip.Active := StrToBool(Grid.Cells[1, i+1]);
      Chip.OpenEvenComplex := StrToBool(Grid.Cells[2, i+1]);
      Chip.CheckCount := StrToInt(Grid.Cells[3, i+1]);
      Chip.Port := StrToInt(Grid.Cells[4, i+1]);

     S := Bin(Grid.Cells[0, i+1]);
     V := Chip.Value;
