...
uses
  ...
  Dialogs, StdCtrls, Grids, ComCtrls, ExtCtrls, TSSServContAPI;
...
type
  ...
  public
    { Public declarations }
    procedure Timetable2Controls(const SpecialDays, Items: string);
    procedure Controls2Timetable(var SpecialDays, Items: string);
  end;
...
procedure TfrmTimetable.Timetable2Controls(const SpecialDays, Items: string);
var
  i, Count, y: Integer;
  pDay: PControllerTimetableSpecialDay;
  pItem: PControllerTimetableItem;
begin
  ...
  if Count <> 0 then begin
    ...
    for i:=0 to Count - 1 do begin
      ...
      sgdItems.Cells[1, y] := Format('%.2d:%.2d', [pItem^.Start.Hour, pItem^.Start.Minute]);
      sgdItems.Cells[2, y] := Format('%.2d:%.2d', [pItem^.Fihish.Hour, pItem^.Fihish.Minute]);
...
procedure TfrmTimetable.Controls2Timetable(var SpecialDays, Items: string);
var
  i, Count: Integer;
  s: string;
  Day: TControllerTimetableSpecialDay;
  Item: TControllerTimetableItem;
begin
  Count := sgdSpecialsDays.RowCount - 2;
  i := SizeOf(TControllerTimetableSpecialDay) * Count;
  SetLength(SpecialDays, i);
  for i:=1 to Count do begin
    Day.Year := StrToInt(Copy(sgdSpecialsDays.Cells[0, i], 1, 4)) - 2000;
    Day.Month := StrToInt(Copy(sgdSpecialsDays.Cells[0, i], 6, 2));
    Day.Day := StrToInt(Copy(sgdSpecialsDays.Cells[0, i], 9, 2));
    Day.DayType := StrToInt(sgdSpecialsDays.Cells[1, i]);
    Move(Day, SpecialDays[SizeOf(TControllerTimetableSpecialDay) * (i - 1) + 1], SizeOf(TControllerTimetableSpecialDay));
  end;

  Count := sgdItems.RowCount - 2;
  i := SizeOf(TControllerTimetableItem) * Count;
  SetLength(Items, i);
  for i:=1 to Count do begin

    Item.DayType := StrToInt(sgdItems.Cells[0, i]);

    s := sgdItems.Cells[1, i];
    Item.Start.Hour := StrToInt(Copy(s, 1, Pos(':', s) - 1));
    Item.Start.Minute := StrToInt(Copy(s, Pos(':', s) + 1, MaxInt));

    s := sgdItems.Cells[2, i];
    Item.Fihish.Hour := StrToInt(Copy(s, 1, Pos(':', s) - 1));
    Item.Fihish.Minute := StrToInt(Copy(s, Pos(':', s) + 1, MaxInt));

    Item.PersCat := StrToInt(sgdItems.Cells[3, i]);
