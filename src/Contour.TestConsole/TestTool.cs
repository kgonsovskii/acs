...
public class TestTool
{
    ...
    public async Task StartPolling()
    {
        ...
        await channel.Open();
        var spot = new SevenSeals.Tss.Contour.Contour(channel, CancellationToken.None, 171);
        spot.Activate();
        int eventCount = 0;
        ...
        {
            if (evt.Kind == ControllerEvent.KindEnum.Key || evt.Kind == ControllerEvent.KindEnum.DoorClose)
            {
                eventCount++;
                var logMessage = $"ControllerEvent: {evt.Kind} Controller: {evt.ChannelId}, Address: {evt.Address}, Time: {evt.ControllerTimestamp}";

                // Add key hex string for key events
                if (evt is ControllerKeyEvent keyEvent)
                {
                    logMessage += $", Key: {keyEvent.KeyHexString}";
                }

                Log(logMessage);
            }

            // Enrich logging for base ControllerEvent (when it's not a specific subclass)
            if (evt is ControllerEvent && 1 ==0)
            {
                Log($"=== Base ControllerEvent Details ===");
                Log($"Kind: {evt.Kind}");
                ...
                Log($"=== End Base ControllerEvent Details ===");
            }
        };
