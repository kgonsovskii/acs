...
public class ControllerSendableEvent : SendableEvent
{
    ...
    public ControllerSendableEvent(string name, ControllerEvent evt) : base(name)
    {
        ...
        Array.Copy(evt.GetControllerTimestamp().ToBinary(), 0, Data, 0, 8);
        Data[6] = evt.GetAddress();
        BitConverter.GetBytes(evt.GetNo()).CopyTo(Data, 7);
        Data[9] = (byte)(evt.IsAuto() ? 1 : 0);
        Array.Copy(evt.GetControllerTimestamp().ToBinary(), 0, Data, 10, 8);
