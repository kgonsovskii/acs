...
namespace Gui.WinForms.Forms
{
    public partial class ContourForm : Form
    {
        ...
        public ContourForm(IContourClient client, ISpotClient spotClient)
        {
            ...
            btnEvents.Click += async (s, e) => await HandleEventsRequest();
        ...
        private async Task HandleEventsRequest()
        {
            try
            {
                ...
                if (ShowPropertyDialog(request, "Events Request") == DialogResult.OK)
                {
                    var response = await _client.Events(request);
                    MessageBox.Show(response?.ToString() ?? "No response", "Events Response");
                }
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Error executing events request: {ex.Message}", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
