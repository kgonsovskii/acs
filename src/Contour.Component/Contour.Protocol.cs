...
public partial class Contour
{
      ...
      //  ++Channel._speedCounter;
        ...
        void _processEvt(byte[] buf)
        {
            ...
            if ((buf[12] & 0x40) != 0 && ProgId == 0x9C)
                throw new ContourException("Error", "Erroneous event");

            //  _setState(_autonomic ? _stateAutonomicPolling : _stateComplex);

            // Legacy raw event for backward compatibility
            OnRawEvent?.Invoke(this, buf);

            // Use factory to create the appropriate concrete ControllerEvent
            var controllerEvent = ControllerEvent.Create(Channel.Id, buf);
            OnEvent?.Invoke(this, controllerEvent);
        }
    }
    ...
    private int ReadPack(byte[] buf, int size, bool checkOp)
    {
        ...
        while (true)
        {
            ...
            if (r == 0)
                throw new ContourException("Timeout", $"No response in {Channel.Options.ResponseTimeout} msec");
    ...
    private int CheckPack(byte[] buf, int size, bool checkOp)
    {
        ...
        if (size > 3)
        {
            ...
            if ((size - 3) >= len)
            {
                var x = len + 2;
                if (Cs8(buf, x) != buf[x])
                    throw new ContourException("Error", "Invalid checksum");
                ...
                if (checkOp && len == 1)
                {
                    ...
                    {
                        case 0xFE:
                            throw new ContourException("Error", "Invalid pack length");
                        case 0xFF:
                            throw new ContourException("Error", "Illegal operation");
                    }
                }
            }
        }
        ...
        {
            throw new ContourException("Error", "Busy");
        }
